...
export default {
  ...
  methods:{
    ...
    {
      this.$vs.loading();
      axios.post('api/getCryptos').then(response => {
    ...
    addToFavorites(row)
    {
      axios.post('api/addToFavorites',{stock: row.item, type: 'crypto'}).then(response => {
        if (response.data)
        {
          this.$vs.notify({title:'Success',text:'The cryptocurrency has been added to your favorites!',color:'success'})   
